<script lang="ts">
  ...
  import VersionPanel from '$lib/components/VersionPanel.svelte';

  const store = editorStore;
  const { nodes, ropes, ropePaths, selectedNodeId, selectNode, selectedRopeId, selectRope, versions } = store;

  let activeTab = $state<'nodes' | 'ropes' | 'versions' | 'legend'>('versions');
